...
try {
  ...
  const result = await page.evaluate(async () => {
    ...
    const watch = await call('configure_route_watch', { trigger: 'ceiling_below_threshold', ceilingThresholdFt: 1500, visibilityThresholdSm: 5, delivery: 'in_app' });
    const change = await call('check_route_changes');
    const validation = await call('validate_weather_alert', { alertId: change.alert.id });
  ...
  if (result.toolNames.length !== 12 || result.assessment.level !== 'HIGH' || result.assessment.goNoGo !== null || result.alternates.alternates.length !== 3 || result.options.preferred !== null || !result.options.requiresPilotChoice || !result.watch.active || result.change.alert.pilotAction !== null || !result.validation.validated || !result.acknowledgment.acknowledged || result.decision.choice !== 'delay') process.exitCode = 1;
  console.log(JSON.stringify({ tools: result.toolNames.length, risk: result.assessment.level, goNoGo: result.assessment.goNoGo, alternates: result.alternates.alternates.length, requiresPilotChoice: result.options.requiresPilotChoice, routeWatch: result.watch.active, alertValidated: result.validation.validated, alertDirectsAction: result.change.alert.pilotAction, alertAcknowledged: result.acknowledgment.acknowledged, recordedDecision: result.decision.choice }, null, 2));
} finally {
  await browser.close();
}
